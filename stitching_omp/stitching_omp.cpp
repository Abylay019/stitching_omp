...
#pragma omp parallel for
	for (int i = 0; i < num_images; ++i)
	{
		...
		resize(full_img[i], img[i], Size(), seam_scale, seam_scale, INTER_LINEAR_EXACT);
...
#pragma omp parallel for
	...
	for (int img_idx = 0; img_idx < num_images; ++img_idx)
	{
		...
		full_img1 = imread(samples::findFile(img_names[img_idx]));
		if (!is_compose_scale_set)
		{
			if (compose_megapix > 0)
				compose_scale = min(1.0, sqrt(compose_megapix * 1e6 / full_img1.size().area()));
		...
		if (abs(compose_scale - 1) > 1e-1)
			resize(full_img1, img1, Size(), compose_scale, compose_scale, INTER_LINEAR_EXACT);
		else
			img1 = full_img1;
		full_img1.release();
	...
	for (int img_idx = 0; img_idx < num_images; ++img_idx)
	{
		...
		// Read image and resize it if necessary
		//full_img[img_idx] = imread(samples::findFile(img_names[img_idx]));
		if (!is_compose_scale_set)
		{
